...
export const MetricsCatalogTab: React.FC = () => {
  ...
  const handleDownloadCatalog = () => {
    ...
    categories.forEach(cat => {
      ...
      if (detail?.purpose) {
        md += `**Purpose:** ${detail.purpose}\n`;
      }
      md += `**Total Metrics:** ${cat.metric_count}\n\n`;

      if (detail) {
        (['High', 'Medium'] as const).forEach(priority => {
          const metrics = detail.metrics[priority];
          if (metrics && metrics.length > 0) {
            md += `### ${priority} Priority (${metrics.length} metrics)\n`;
            md += `| # | Metric Name | Type | Description | Keywords |\n`;
            md += `|---|-------------|------|-------------|----------|\n`;
            metrics.forEach((m, i) => {
              const kw = m.keywords ? m.keywords.join(', ') : '';
              md += `| ${i + 1} | ${m.name} | ${m.type} | ${m.help || ''} | ${kw} |\n`;
            });
            md += `\n`;
          }
        });
